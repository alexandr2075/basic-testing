...
describe('simpleCalculator tests', () => {
  ...
  test('should substract two numbers', () => {
    expect(simpleCalculator({ a: 1, b: 2, action: Action.Substract })).toBe(-1);
  });
  ...
  test('should return null for invalid action', () => {
    expect(simpleCalculator({ a: 1, b: 2, action: 4 })).toBe(null);
  });

  test('should return null for invalid arguments', () => {
    expect(simpleCalculator({ a: 1, b: 'p', action: Action.Add })).toBe(null);
  });
});
